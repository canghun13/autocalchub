# AutoCalcHub 인수인계 문서 (2026-07-15 기준)
...
- **⚠️ 다음 세션 필독**:
  ...
  2. **(수정) 1차 조치 후 사용자 지적으로 재점검 진행** — 처음엔 "링크 0개" 9개 페이지만 처리하고 멈췄었는데, "링크 1개"인 페이지가 23개나 더 있는 걸 방치한 채 끝내려 했던 게 안일했음. 지적받고 2차로 색인된 허브 9개(기존 7개 + `car-insurance-estimator.html`, `how-to-negotiate-car-price-at-dealership.html` 신규 활용)를 통해 23개 중 18개를 추가로 2~4개 링크 확보 상태로 끌어올림. 최종 결과: 미색인 50개(정적 페이지 제외) 중 **0개 링크 페이지 0개, 1개 링크 5개, 2개 이상 45개**로 개선(작업 전: 0개 링크 9개, 1개 링크 23개).
  3. 남은 "1개 링크" 5개(`best-time-of-year-to-buy-a-car`, `how-does-car-depreciation-affect-resale-value`, `how-much-should-i-spend-on-a-car-based-on-salary`, `how-to-lower-your-car-insurance-premium`, `is-it-worth-buying-car-with-rebuilt-title`)는 색인된 허브 페이지들과 자연스러운 주제 연결점이 마땅치 않아 억지로 링크를 밀어넣지 않고 보류함 — 부자연스러운 링크 스터핑은 그 자체로 품질 신호를 해칠 수 있어서, 다음 세션에서 새로운 자연스러운 연결점이 생기면(예: 관련 신규 콘텐츠) 처리.
  4. **재점검 과정에서 헛다리짚을 뻔한 사례**: `car-lease-vs-buy-calculator.html`을 246단어로 잘못 측정해서 "이것도 원인인가" 싶었는데, 추출 스크립트 버그였고 실제로는 1005단어로 정상이었음(재확인 후 폐기). 콘텐츠 분량 자체는 사이트 전체에서 문제 없는 것으로 재확인.
  5. 이번 조치(1차+2차)의 효과가 1~2주 후 Coverage 리포트에 반영되는지 먼저 확인한 뒤, 효과가 있으면 남은 5개 페이지도 순차 처리. 효과가 없으면 "내부링크 결손" 가설도 기각하고 원점 재검토 필요.
  6. Related Guides 리스트에 항목을 추가할 때는 사이드바 리스트 삽입 방식(이번 세션)보다 본문 내 contextual 링크가 이론상 더 강한 시그널이라는 점 참고 — 리스트 삽입으로 효과가 부족하면 다음 단계로 본문 내 링크 삽입 고려.
  7. `car-loan-calculator.html`의 Related Guides가 이번 세션 두 차례 보강으로 8개→14개까지 늘어남 — 사이트 평균(5~8개)보다 확연히 김. 다음 세션에서 시각적으로 리스트가 과하게 길어 보이지 않는지 한 번 확인할 것(기능상 문제는 없으나 UX 관점에서 카드형 레이아웃으로 리팩터링을 고려할 수도 있음).
