...
## 5. SEO / Search Console 현황 (2026-08-07 기준 최신 확인 — 실제 GSC export(Coverage+Performance zip, 8/7 다운로드) + GA4 개요 CSV로 재확인)

### 핵심 지표
- **색인: 16 → 19개로 7주 만에 처음 움직임** (7/11자로 3개 증가). 7/15 내부링크 보강의 효과로 추정되나 표본이 작아 단정 불가.
- **⚠️ 그러나 미색인이 55 → 73개로 더 크게 증가**(발견됨-크롤링대기 71 + 크롤링됨-미생성 2). **색인 속도(월 3개)보다 발행 속도(월 8~12개)가 훨씬 빨라서 격차가 계속 벌어지는 중.** 전체 89개 콘텐츠 페이지 중 색인 21%. 이게 현재 사이트의 **가장 큰 단일 병목**이며, 콘텐츠 부족 문제가 아님.
- **클릭: 3개월 누적 여전히 1건**(7/25 그 1건 그대로, 이후 0). 노출 1,748회.
- **⚠️ 그 1건의 클릭은 사실상 본인 트래픽으로 판단**: 국가별 리포트에서 한국 노출 12회 / 클릭 1회 / 게재순위 1.17위로 잡힘. 미국은 1,632회 노출에 클릭 0. 한국에서 1위로 노출됐다는 건 브랜드명 직접 검색(본인 확인용)일 가능성이 압도적. **즉 실질 오가닉 클릭은 3개월간 0건으로 봐야 하며, 이전 세션에서 "첫 클릭 발생 🎉"이라고 기록한 건 과대 해석이었음. 앞으로 클릭 지표 볼 때 반드시 국가/순위 같이 확인할 것.**
- **노출 정체**: 최근 2주 일별 노출 0~4회. 5월 초기 크롤링 스파이크(하루 200~270회) 이후 3개월째 같은 baseline.

### 페이지·쿼리 레벨
- 상위 페이지 순위는 지난 확인 대비 사실상 전부 동일: how-much-car-can-i-afford 19.39위/노출110(완전 동일), car-loan-calculator 4.06위, road-trip 9.53위, car-insurance-estimator 85.6위(노출 1,079). how-to-negotiate-car-price-at-dealership만 48.15→54위로 추가 하락(표본 15회, 여전히 무의미).
- **20위 이내 쿼리가 사이트 전체에서 17개뿐이고, 그중 의미 있는 노출(2회 이상)은 6개**. 그 6개 중 5개가 전부 `car affordability rules of thumb` 계열 — 2.17위(노출6), 3.64위(노출11), 5.5위(노출2) 등. **사이트에서 유일하게 검증된 승리 패턴이며, 공통점은 "연도 수식어 + 아무도 안 쓰는 특이 표현(rules of thumb)" 조합의 초장꼬리 쿼리.**
- 반대로 헤드텀은 전부 죽어 있음: "how much car can i afford" 93.5위, "car depreciation calculator" 74.93위, "mpg calculator" 70.18위. **같은 페이지가 초장꼬리에선 2~5위, 헤드텀에선 70~93위** — 도메인 권위가 없으면 헤드텀은 콘텐츠 품질과 무관하게 안 된다는 증거.
- 감가상각 클러스터(car depreciation calculator 노출30/74.93위, how much will my car depreciate 노출17, vehicle depreciation calculator 노출15, auto depreciation calculator 노출13 등 합산 약 95회)는 노출은 두 번째로 많지만 전부 73~88위 — **보험 클러스터와 동일한 "노출만 많고 순위는 죽은" 패턴. 추가 투자 대상 아님.**
- 보험 클러스터: 전체 쿼리 374개 중 146개(39%)가 보험 관련, 노출 1,079회로 사이트 1위지만 80~94위 정체. 방치 정책 계속 유지.

### GA4 (7/10~8/6, 4주)
- 활성 사용자 55명(직전 4주 51명과 사실상 동일), 평균 참여시간 23초로 오히려 감소(32초 → 23초).
- 세션 소스: (direct) 38, **bing organic 7, claude.ai 7, yahoo 4, google organic 3**. 구글 오가닉이 빙보다 적음 — 색인 상태를 그대로 반영.
- 지역 1위 여전히 Busan(5명) + foundrlist/tryrating/newtool.site 디렉토리 리퍼럴 7명 — **직전 세션에 기록한 "GA 트래픽 상당수가 본인 테스트 + 디렉토리 크롤러" 판단 그대로 유효. GA 페이지 랭킹을 콘텐츠 인기 신호로 쓰지 말 것.**
- 참고로 "What Is a Good MPG for a Car?"가 조회 14회로 GA 2위지만 이탈률 85%이고 GSC 노출은 미미 — 위 잡음 이슈의 전형적 예시.

### 📌 여기서 도출되는 전략적 결론 (다음 세션들에서 계속 유효)
1. **콘텐츠 양은 병목이 아니다.** 89개 중 70개가 색인조차 안 됐고 콘텐츠 볼륨 중앙값은 1,049단어로 이미 충분함(900단어 미만 16개뿐, 대부분 계산기 페이지라 정상). 여기서 페이지를 더 늘리는 건 크롤링 예산만 분산시켜 **역효과**.
2. **병목은 도메인 권위 + 크롤링 예산.** 해결 수단은 콘텐츠가 아니라 (a) 발행 중단/감속, (b) 외부 백링크, (c) 시간.
3. **초장꼬리 전략은 실제로 작동한다** — 단, 그 대가로 쿼리당 노출이 2~11회 수준이라 **애드센스 같은 볼륨 기반 수익모델과는 궁합이 나쁨.** 클릭당 단가가 높은 제휴 모델로 가야 이 트래픽 구조에서 수익이 남음(아래 5-1 참고).
...
## 6-12. 8/7 세션: GSC/GA 분석 결과 **신규 콘텐츠 작업 없음으로 결론** + 수익화 정책 확정

- **결론: 이번 주는 콘텐츠 발행 안 함.** 규칙 9("작업할 게 없으면 억지로 만들지 말 것") 적용. 근거는 위 섹션 5의 "전략적 결론" 3개 항목 — 요약하면 색인 21%(89개 중 19개) 상태에서 페이지를 더 늘리면 크롤링 예산만 분산돼 역효과. **주간 루틴(블로그 3 + 툴 1)을 이번 주는 의도적으로 건너뜀.**
- **⚠️ 이건 일회성 스킵이 아니라 정책 제안임**: 색인률이 의미 있게 올라오기 전까지(대략 색인 40~50% 또는 절대수 35개 이상) **신규 발행을 중단하거나 대폭 감속하는 것을 권장.** 다음 세션에서도 GSC 색인 수를 먼저 보고, 안 올라왔으면 또 발행하지 말 것. 사용자 승인 필요한 사안이라 8/7 세션에서 제안만 하고 확정은 안 함.
...
- **수익화 정책 확정** — 사용자 지시로 섹션 5-1 신설(애드센스 비의존, 제휴 전방위 검토, 재심사·가입·철수는 Opus가 실익 판단, 애드센스보다 나은 대안이면 추천). 관련해서 **애드센스 재심사는 지금 넣지 말 것으로 판단**(색인 21% 상태에선 같은 판정 반복 가능성 + 승인돼도 현 트래픽에서 실익 $0 수준). 상세 근거와 제휴 후보 리스트는 5-1 참고.
- **리서치는 했으나 발행으로 이어지지 않음**: 오토 리파이낸스 프리퀄리피케이션 영역을 후보로 검토했으나(우리 사이트 "car refinance pre approval" 3.33위 신호 + 제휴 단가 최상위 $60~150), 웹서치 결과 Caribou/RefiJet/PenFed/Patelco 등 **대출기관 본인들이 상위를 직접 장악**하고 NerdWallet까지 붙어 있어 규칙 4의 회피 기준에 해당 — 기각. 3.33위 신호는 노출 3회짜리 표본이라 의미 없음으로 재확인.
- 사용자가 8/6에 직접 커밋(`9e2a3ca`)으로 `index.html`에 boostdomainrating 배지 추가 — 백링크 작업을 사용자가 병행 중. 이 방향은 현재 병목(도메인 권위)에 정확히 맞는 조치라 계속 권장.
- **다음 세션 확인 사항**: (1) 색인 수가 19개에서 더 올라갔는지 — 이게 발행 재개 여부의 판단 기준, (2) 7/27 발행분 3개(EV 등록세, 리스 테이크오버 블로그+계산기)의 색인 여부(8/7 Coverage는 7/24 컷오프라 아직 미반영), (3) 애드센스 재심사 결과 확인 여부.
