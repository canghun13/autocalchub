...
4. **사용자 액션이 필요한 최고 레버리지: GSC URL 검사 → "색인 생성 요청" 수동 제출.** 하루 약 10건 쿼터, 81개면 8~9일. 지금 무료로 할 수 있는 것 중 유일하게 크롤링을 강제하는 수단. 우선순위는 (a) 8/11 신규 4건 (b) 노출 있는 17개와 주제가 붙어있는 페이지 (c) 나머지.
...
### ⚠️ 다음 세션 필독
0. **사용자에게 GSC "색인 생성 요청" 수동 제출을 요청할 것** — 하루 10건 쿼터, 8/11 신규 4건부터. 현재 크롤링 병목을 뚫는 유일한 무료 수단이며, 이걸 안 하면 발행량과 무관하게 색인은 계속 정체됨.
