# AutoCalcHub 인수인계 문서 (2026-10-26 기준)
...
## 2. 콘텐츠 현황 (2026-10-26 기준)
- 블로그 83개, 툴 37개 (index.html 제외). 10/26 세션에서 블로그 1 + 툴 1 추가.
- sitemap.xml 총 URL 133개
...
## 5. SEO / Search Console 현황 (2026-10-26 확인 · 구글 9/21자, Bing 9/21자)
...
### 구글 (9/21자) — 정체 지속
- **미색인 101 → 104개, 전부 최종크롤링 1970-01-01.** 차트가 9/4에서 9/18로 갱신된 것이 유일한 변화.
- 3개월 노출 128 → 126, 노출 페이지 24개, 클릭 1건(홈페이지). 사실상 무변동.
- **running/expense·break-even 군집 9쿼리 3주 연속 소수점까지 동일**("car break even calculator" 8노출/89.12위). 10/5 보강이 미크롤링 때문에 영원히 반영되지 않는 상태 — 추가 조치 불필요, 다음 자료에서도 확인만 할 것.
- 코사이너 페이지 13노출/84.08위 유지. 구글 쪽 코사이너 수요는 살아있으나 순위가 80위대라 단기 개선 여지 없음.

### 🎉 Bing 계속 급성장 (9/13 → 9/21, 8일)
- **노출 1,933 → 2,668(+38%), 클릭 38 → 51(+34%), 페이지 35개.**
- 클릭 기여 상위:
  | 페이지 | 노출 | 클릭 | 순위 |
  |---|---|---|---|
  | how-much-should-i-spend-on-a-car-based-on-salary | 363 | **14** | 3.73 |
  | how-much-should-i-put-down-on-a-car | 376 | **10** | 4.27 |
  | **how-much-does-it-cost-to-fill-up-a-car** | 421 | **9** | 5.30 |
  | how-to-get-pre-approved-car-loan | 27 | **3** | 3.59 |
  | what-is-a-good-mpg-for-a-car | 874 | 2 | 5.64 |
- **fill-up 페이지가 폭발적 성장**: 189 → 421노출(2.2배), 5 → 9클릭. **차종 특정 쿼리가 유입 중**("2026 buick envision", "2026 xc90", "midsize suv", "gmc suburban"). 연봉·다운페이먼트에 이은 3번째 전환 엔진.
- **사전승인 페이지 성장 확인**(10/19 이월과제 #3 적중): 11 → 27노출, 1 → 3클릭, 4.55 → 3.59위. 클릭 쿼리 4건 전부 1~8위.
- MPG 874노출(사이트 33%)/5.64위/클릭 2. CTR 0.23%. **3번째 메타 수정 금지 방침 유지.**
- salary + put-down + fill-up **3개 페이지가 클릭 51건 중 33건(65%)**.

### Bing 신규 신호 (10/26 분석)
- **차종·모델 특정 주유비 쿼리가 계속 늘어남**: "how much does it cost to fill of gas 2026 buick envison"(1위/클릭), "what does it normally cost to fill a 1/2 of gas in a 2026 xc[90]"(1위/클릭), "how much does it cost to fill up a tank of a midsize suv in [us]"(3위/클릭). **차종 입력형 도구 각도가 여전히 유효**(콘텐츠 팜 회피).
- 판매세 쿼리 클릭: "when you buy a new car is sales tax based off of the sales p[rice]"(1위/클릭) — 과세표준 질문.
- 다운페이먼트 금액 특정: "42210 is a car price how much should i put down"(3위/클릭).
- 연봉 특정: "if i make 165000 how much should i spend on a car"(2위/클릭).

### 📌 전략적 결론 (10/26 갱신)
1. **Bing 3대 엔진(연봉·다운페이먼트·주유비)이 클릭 65%.** 이 셋을 넓히는 것이 가장 확실한 투자.
2. **사전승인이 4번째 엔진으로 진입 중** — 27노출/3클릭/3.59위. 10/26에 딜러 마크업 각도로 확장함.
3. **콘텐츠 모양은 의사결정형 고정**(9/21 발견, 이후 매주 재확인).
4. **구글은 크롤링 병목으로 정체.** 색인된 24개 페이지 밖의 작업은 구글 쪽 효과를 기대하지 말 것.
5. **MPG 메타 재수정 금지**(3회째 금지).
6. **🚫 "GSC 색인 생성 요청 수동 제출" 제안 금지**(8/11 사용자 명시 기각).
...
## 6-19. 9/21 세션 (주간 루틴): CTR 진단 반전 + 이월 과제 소화

- **⚠️ 첨부 사고**: 처음 업로드 폴더에 신규 자료가 없었고(최신이 8/24자), 세션 중반 사용자가 "실수로 다른 프로젝트를 줬다"며 **Bing 8/30 자료만 재전달**. 구글 GSC·GA4는 8/24자가 여전히 최신이므로 그것으로 판단함.
- **Bing 8/30 분석 결과 9/7 진단이 반증되어 5번 섹션을 전면 개정.** 핵심은 "CTR 문제는 메타가 아니라 쿼리의 모양" — 정의형 0.00% vs 의사결정형 5.33%.

### 작업 1 — `blog/how-much-does-car-maintenance-cost-per-year.html` 보강 (Bing 28노출/5.50위)
- Bing 최다 쿼리가 "average ... per year" 형태인데 타이틀에 "average"가 없었음 → `Average Car Maintenance Cost Per Year (2026) — Including Tires`로 교체.
- **"The Average, in One Number" 섹션 신설**: 연 $900~1,200이 기준선이며 **타이어는 대개 제외돼 있다**는 점(연 $150~300 추가 → 현실치 $1,050~1,500)을 명시. Bing 쿼리 "average maintenance and tires cost for car per year"(6위)에 직접 대응.
- ⚠️ 단, 8/30 진단에 비춰보면 "average maintenance cost" 자체는 정의형이라 클릭 기대는 낮음. 다만 이 페이지에는 **모델 특정 쿼리**("2017 dodge minivan" 3위, "jetta elantra sentra k4")가 붙고 있어 그쪽이 전환 가능성이 높음 — **다음에 보강한다면 차종별 유지비 각도로 갈 것.**

### 작업 2 — `tools/car-total-cost-of-ownership-calculator.html` 커버리지 확대 (9/7부터 이월)
- 구글 10쿼리(car running costs / vehicle expense / auto expense / car break even / car savings 등, 84~100위)에 대해 **해당 용어가 본문에 0회**였음.
- 타이틀을 `Car Running Cost Calculator — Total Vehicle Expense & Break-Even (5-Year)`로 교체, 디스크립션도 재작성.
- **"Running Costs vs Purchase Price" + "Finding the Break-Even Between Two Cars" 두 섹션 신설.** 쿼리 언어(running cost 7회, vehicle expense 3회, break-even 4회, cost per mile/year)를 실제 본문에 포함. "car break even calculator"는 구글 최다 노출 쿼리(5)였음.

### 작업 3 (신규) — `blog/how-much-does-an-accident-affect-trade-in-value.html` (Accidents & Claims, 1,195단어)
- 근거: Bing 8/24 "how much would it affect the trade-in value of a brand new car with 700 miles and $1,000 in repairs"(2쿼리 4노출/5~8위), 8/30 "how much can i get for a trade for my 2018 bmw x5 that needs..."(2노출/7.5위). **전용 페이지 0개**였고 `is-a-diminished-value-claim-worth-filing`은 보험 청구 관점뿐.
- 경쟁: 법률사무소(dubofflaw·schaarsilva·bartdurham)·감정사 사이트가 **DV 청구 리드젠 목적**으로 10~30% 손실을 주장. Carfax·nexusautotransport 등이 다른 숫자.
- **경쟁 회피 각도 — 두 숫자의 모순을 정면으로 해부**: 법률사무소 10~30% vs **Carfax 자체 시장데이터 평균 $500 미만(중대손상 $2,100)**. 3년차 $28,000 차 기준 3.1% vs 20%로 6배 차이. 아무도 이 모순을 설명하지 않음 → 우리가 "서로 다른 질문에 답하고 있다"로 정리하고 손상 등급별 표를 제시.
- 검증된 인사이트: **퍼센트는 유지되고 달러 손실은 줄어든다**(1년차 $3,360 → 7년차 $1,267) → "기다리면 희석된다"는 통념 반박(그 사이 감가로 $13,952 손실, 5배 이상). **수리비는 가치 손실의 지표가 아니다**(구조손상 여부가 결정, 신차 $1,000 수리가 $4,600 손실).
- **의사결정형 콘텐츠 모양** — 8/30 발견과 정확히 일치하는 방향. 4가지 출구(DV 청구/개인매각/여러 곳 견적/수리 문서화) 제시.

### 사이트 반영
- blog/index.html Latest + Accidents & Claims, index.html 미리보기 3건 교체.
- sitemap 121→122 URL(lastmod 11건), llms.txt Accidents 1건 + TCO 설명에 running cost/vehicle expense/break-even 문구 추가.
- 내부링크 13개 페이지(신규 블로그 10곳 + TCO 계산기 3곳). 신규 블로그 인바운드 12개.

### 검증
- 17개 파일 태그 밸런스·JSON-LD 통과, sitemap XML 유효(122 URL), 모바일 `.row` 미디어쿼리 검사 통과, 내부링크 1,122개 전수 스캔 깨짐 0건.
- 템플릿 CSS 정규식 경계 추출 + 오염검사 적용(8/31 사고 방지) → 오염 없음.

### ⚠️ 다음 세션 필독
1. **콘텐츠 기획 시 "의사결정형 쿼리 모양"을 최우선 기준으로 삼을 것.** 정의형(what is X)은 노출만 쌓이고 클릭이 0이다. 제목과 H2를 "should I / how much for my situation" 형태로 잡을 것. 이번 데이터가 가장 강하게 말해주는 지침.
2. **MPG 페이지 메타 재수정 금지**(2회 실패). 370노출/0클릭은 쿼리 구조 탓이며 고칠 수 없음.
3. **차종·금액이 박힌 쿼리가 늘고 있음**("2017 dodge minivan", "jetta elantra sentra k4", "2018 bmw x5", "gmc yukon 1000 miles"). 차종별 유지비/연료비 각도는 아직 미커버 — **다음 클러스터 후보 1순위.**
...
## 6-21. 10/5 세션 (사용자 지시: 신규 위주, 폭넓은 키워드 리서치): 신규 클러스터 "Business & Tax" 신설

- **지시**: 신규 중심, 키워드 폭넓게 → 리스트 → 경쟁강도 → 강하면 롱테일 우회. 새 클러스터 환영.
- **GSC/GA/Bing 신규 자료 없음** — 외부 리서치 기반. Bing 8/30 판단(5번 섹션) 그대로 유효.

### 후보 리스트 → 경쟁강도 판정 (웹서치 3라운드)
| 후보 | 상위 노출 | 판정 |
|---|---|---|
| Turo 호스팅 수익성 | carshare.substack.com(Carshare Chronicles)이 전문 매체로 장악, 코스 판매 병행 + 유튜브 다수 | **토픽 권위 강함 → 기각** |
| 마일리지 공제 헤드텀 | TurboTax·Nolo·Ramp·driversnote·MileIQ·ustax.tools·nationaltaxtools·shifttracker | **포화 → 각도 전환** |
| **1년차 선택의 영구 락인** | 대부분 각주로만 언급, **다년 비교를 계산해주는 곳이 사실상 없음** | **진입 (핵심 웨지)** |
| 차량 기부 세금공제 | 미조사(이월) | 다음 후보 |

### 경쟁 회피 장치 — "1년차 비교가 함정이다"
- 경쟁 콘텐츠는 전부 **단일 연도 비교**(올해 실비 vs 올해 마일리지)를 보여줌. 그런데 **1년차에 실비법을 택하면 그 차량은 영구히 표준마일리지를 못 쓴다**(Pub 463). 감가상각은 1년차에 몰려 있으므로 **단일 연도 비교는 거의 항상 실비법을 유리하게 보이게 만든다** — 즉 가장 흔한 비교 방식이 되돌릴 수 없는 오선택을 유도한다.
- **검증된 수치**: $28,000 승용차 / 사업 80% / 연 15,000 사업마일 →
  - 1년차만: 표준 $11,137 vs **실비 $18,880 (실비 승)**
  - 5년 합계: **표준 $56,738 (승)** vs 실비 $49,600
  - 답이 뒤집히고, 1년차 숫자를 따르면 $7,138 손실 + 선택권 영구 상실.
- 반대로 **헤비 SUV $62,000 / 연 8,000 사업마일**은 1년차·5년 모두 실비 압승($85,529 vs $30,260) — §179 $30,500 + 보너스 40%가 승용차 럭셔리 캡을 우회하기 때문. 즉 "실비가 항상 함정"이 아니라 **계산이 필요한 문제**라는 정직한 결론.

### 검증된 2026 세법 사실 (다음 세션 재사용 가능)
- **2026 표준율은 연중 변경**: 1/1~6/30 **72.5¢**, 7/1~12/31 **76¢**(균등 주행 시 블렌드 74.25¢, 단 로그를 6/30에 나눠 적용해야 정확). 의료·이사 23.5¢(7/1~), 자선 14¢(의회 입법 고정).
- **1년차 락인**: 보유 차량은 1년차에 표준을 택해야 이후 양방향 전환 가능. 실비를 먼저 택하면 표준 영구 불가.
- **리스는 더 엄격**: 표준을 택하면 **갱신 포함 리스 전 기간** 표준 유지. 리스엔 감가상각 없고 리스료의 사업분을 공제.
- §179 헤비 SUV(>6,000 GVWR) 2026년 상한 **$30,500**, 보너스 감가상각 **40%**(OBBBA 스케줄), 승용차 1년차 상한 약 **$20,300**.
- **5대 이상 동시 사용 시 표준 사용 불가**. 같은 차량·같은 해에 방식 혼용 불가(차량별로는 다르게 가능).
- **통근은 절대 공제 불가**. 단 적격 홈오피스가 주된 사업장이면 첫 이동부터 사업 주행.
- **W-2 직원은 현행법상 미상환 차량비 공제 불가**(사실상 자영업자·사업주 주제).
- 사업사용률 50% 미만 하락 시 **감가상각 리캡처** 발생. §274(d) 동시기록 로그 요구(사후 재구성 로그는 부인 대상).

### 신규 툴 — `tools/mileage-deduction-method-calculator.html` (802단어)
- 차량가·차종(승용/헤비)·사업마일·사업사용률·운영비·보유/리스 입력 → 양 방식 5년 비교.
- **"⚠️ The Year-One Trap" 전용 카드**: 1년차 단독 결론과 5년 결론을 나란히 출력하고, **둘이 상충하면 행을 빨간색으로 표시하고 락인 경고 박스에 손실액을 명시**. 이게 툴의 존재 이유.
- 리스 선택 시 별도 경고(감가상각 0, 리스 전기간 고정 규칙, CPA 확인 권고)로 분기.
- 세무 주제이므로 **디스클레이머를 강하게** 작성(추정치·MACRS 단순화·리캡처·직원 불가·CPA 확인).
- 기존 툴과 로직 미중복 확인: `car-loan-interest-deduction-calculator`는 OBBBA 개인 이자공제, `rideshare-car-cost-per-mile-calculator`는 마일당 원가. 규칙 4 준수.
- 검증: 파이썬 독립 구현 + node 목업 DOM **4시나리오 전부 일치**(승용차 15k/헤비SUV 8k/승용차 8k/리스), 상충 감지 정상 작동.

### 신규 블로그 2건
1. `blog/standard-mileage-vs-actual-expenses.html` (1,145단어) — 락인 규칙, 1년차 vs 5년 대조표, 헤비 SUV 반례, 2026 연중 요율 변경, 통근·직원·주차/통행료·대출이자 처리.
2. `blog/what-car-expenses-can-you-write-off.html` (958단어) — 자격 게이트(직원 vs 자영업자), 통근 규칙과 **홈오피스 예외**, 사업사용률과 리캡처, 방식별 공제 항목표(**주차·통행료·대출이자는 표준율 위에 추가 공제**), §274(d) 기록 요건.

### 사이트 반영
- blog/index.html·tools/index.html에 `💼 Business &amp; Tax` 카테고리 신설(Trucks 뒤/앞). **div 밸런스 166/166 확인.**
- index.html 미리보기 3건 교체 + 툴카드, stat `34+`→`35+`.
- sitemap 125→128 URL(lastmod 10건), llms.txt `## Guides: Business & Tax` 섹션 신설 + Tools 1.
- 내부링크 12개 페이지(기존 OBBBA 이자공제 클러스터·기그 클러스터와 상호 연결). 인바운드 6/10/10.

### 검증
- 템플릿 CSS 정규식 경계 추출 + 오염검사 → 오염 없음.
- 18개 파일 태그 밸런스·JSON-LD 통과, sitemap XML 유효(128 URL), 모바일 `.row` 미디어쿼리 통과, 내부링크 1,200개 전수 스캔 깨짐 0건.

### ⚠️ 다음 세션 필독
1. **의사결정형 쿼리 모양 원칙 유지**(9/21 발견). 이번 3건 모두 의사결정형.
2. **MPG 페이지 메타 재수정 금지**(2회 실패).
3. **Business & Tax는 시즌성이 큼** — 1~4월(신고 시즌)에 확장하면 효율이 가장 높다. 확장 후보: 차량 기부 세금공제, S-corp 차량 소유 vs 개인 소유 후 상환, 홈오피스 마일리지, 분기 예납. **2026 표준율이 연중 변경됐으므로 2027년 초에 요율 갱신 필수**(위 검증 사실 참고).
4. **차종별 각도는 여전히 미착수** — 차종 입력형 도구로 접근할 것(콘텐츠 팜 리스크 회피).
...
## 6-23. 10/19 주간 세션: 연봉 클러스터 역방향 보강 + 10/5 판단 오류 규명

- **자료**: 구글 GSC 9/14자, GA4 개요, **Bing 9/13자**. zip에 8/11~9/7 과거분과 `freetooldev_com`이 또 섞여 있었으나 제외(5회 연속).

### ⚠️ 먼저: 10/5 작업이 무효였음을 확인하고 원인 규명
- 이월과제 #3(구글 running/expense·break-even 군집 순위 변동 확인)을 실행한 결과 **9개 쿼리 전부 소수점까지 무변동**.
- 원인은 **TCO 계산기가 미크롤링 상태**였던 것. 8/11에 확립한 "미색인 페이지 보강은 효과 0" 원칙을 10/5에 확인하지 않고 작업한 내 판단 오류.
- **5번 섹션에 재발 방지 절차를 명문화**(보강 전 GSC 페이지목록 + Coverage 미크롤링목록 양쪽 확인). 이번 세션의 가장 중요한 산출물.

### 작업 선정 근거 (수익화 우선순위)
- Bing 클릭 38건 중 **19건이 salary + put-down 두 페이지**. 이월과제 #1이 정확했고 계속 유효.
- **그 안에서 미커버 방향 발견**: 클릭된 쿼리에 "how much should my slary be for a 56000 dollar car"(2위), "if im kaing 70k before tax..."(2위)가 있는데 **우리 콘텐츠는 전부 연봉→차값 방향이고 차값→필요연봉 역방향이 전무**했음.
- 경쟁 확인(웹서치 1라운드): 정방향은 Autotrader·Edmunds·CarSaver·CarWhere·whatcarcaniafford.com으로 포화. **역방향도 다루긴 하나 답이 제각각**($50k 차 → Autotrader식 3배, AOL $160k, whatcarcaniafford $150~180k) **이고 아무도 규칙별로 화해시키지 않음** → 진입.

### 검증된 핵심 수치 — 역방향은 약 3배, 저가차일수록 배수가 커진다
운영비 월 $350 / APR 7.2% 기준 20/4/10이 요구하는 연봉:
| 차값 | 필요 연봉 | 배수 |
|---|---|---|
| $25,000 | $99,694 | **4.0×** |
| $50,000 | $157,388 | 3.1× |
...
- **핵심 인사이트**: 운영비가 차값과 무관하게 고정($350)이므로 **저가차일수록 10% 상한에서 운영비 비중이 커져 배수가 악화**된다. 즉 "싼 차를 사면 상한을 피할 수 있다"는 통념이 틀렸고, 일정 소득 밑에서는 규칙이 "차 소유 자체가 무리"라고 말하는 셈.
- 규칙 간 격차는 차값 $50,000에서 $50,000(1년치 연봉) ~ $157,388(20/4/10)로 3.1배.

### 작업 1 — `tools/car-affordability-rules-comparison.html`에 역방향 모드 추가 (신규 툴 X)
- **신규 페이지를 만들지 않고 10/12 툴에 방향 선택(정방향/역방향)을 추가.** 수학적으로 역함수 관계라 별도 페이지를 만들면 카니발라이제이션 위험이 컸음. 규칙 4 준수.
- 역방향: 차값 입력 → 6규칙이 각각 요구하는 연봉, 현금 선택 시 추천 전환(20/4/10 → Half your salary), 필요연봉÷차값 배수와 그 배수가 저가차에서 커지는 이유를 노트에 설명.
- 검증: 파이썬 독립 구현 + node 목업 DOM 4시나리오 일치(역방향 $56k/$25k/$100k현금/정방향 복귀).

### 작업 2 (신규) — `blog/what-salary-do-you-need-for-a-50k-car.html` (Buying, 1,112단어)
- 차값 6구간 × 4규칙 역방향 표, 배수 악화 구조, 20/4/10이 왜 그렇게 많이 요구하는지 분해($50k 차 = 월납 $962 + 운영비 $350 = $1,312 → 10% 상한 → $157,388), 소득이 부족할 때 효과 순서(싼 차 > 다운페이먼트 > 금리 > 운영비 > 기간연장은 최후), 현금 구매자 지침(이자 $9,500 차이).
- **의사결정형 모양 유지** — 제목·H2 전부 "내 상황에서 얼마" 형태.

### 사이트 반영
- blog/index.html Latest + Buying a Car, index.html 미리보기 3건 교체(툴 신규 없으므로 stat 36+ 유지).
- sitemap 130→131 URL(lastmod 10건), llms.txt Buying 1건 + Tools 설명에 양방향 문구 추가.
- 내부링크 10개 페이지(전부 구글/Bing 노출 확인 페이지). 인바운드 12개.

### 검증
- CSS 정규식 경계 추출 + 오염검사 통과. 14개 파일 태그 밸런스·JSON-LD 통과, sitemap XML 유효(131 URL), 내부링크 1,247개 전수 스캔 깨짐 0건.

### ⚠️ 다음 세션 필독
1. **보강 작업 전 크롤링 여부 확인(5번 섹션 규칙).** 10/5 실수 재발 방지. 구글 대상 보강은 색인된 24개 페이지에만 의미 있음.
2. **연봉·다운페이먼트 클러스터 계속 확장.** 남은 각도: 가구 합산 소득 기준, 첫 차 예산, 마일당 비용 프레임("how much does it cost for a typical car per mile" 클릭 확인).
3. **사전승인 클러스터가 새 전환 후보** — Bing 2쿼리 클릭(둘 다 1위), 페이지 11노출/1클릭/4.55위, 구글 6노출/23위. **Bing/구글 양쪽에서 살아있고 콘텐츠가 1개뿐**이라 확장 여지가 가장 크다.
4. **정비비 클러스터 성장 중** — car-maintenance-cost-calculator 48→96노출, "auto cost and maintenance calculator"·"average car maintenance cost 2026" 클릭 발생.
5. **MPG 메타 3번째 수정 금지.** 739노출/1클릭은 쿼리 구조 탓.
6. 이월: 차량 기부 세금공제, 겨울타이어, Trucks 확장(트레일러 구입 vs 렌트·토우 패키지·견인 연비), Repairs 확장(타이어 2 vs 4·정비 견적 읽는 법), Accidents 확장(렌터카 커버리지·ACV 이의제기). `car life expectancy`는 전문 사이트 포화라 헤드텀 회피.
7. blog/index.html Latest 트리밍 여부 15세션째 미결.


## 6-24. 10/26 주간 세션: 사전승인 클러스터 확장 — 딜러 금리 마크업

- **자료**: 구글 GSC 9/21자, GA4 개요, Bing 9/21자. 분석은 5번 섹션에 반영.

### 작업 선정 근거 (수익화 최우선)
- **10/19 이월과제 #3이 적중**: 사전승인 페이지가 11→27노출, 1→3클릭, 4.55→3.59위로 성장. 클릭 쿼리 4건이 전부 1~8위.
- **수익화 관점에서 최상위 주제** — 오토론 사전승인/금리는 5-1 정책상 리파이낸스·대출 마켓플레이스 제휴(단가 최상위 $60~150)와 직결. 구글에도 `car refinance pre approval` 3.33위가 계속 유지되고 있음.
- **그러나 기존 `how-to-get-pre-approved-car-loan`이 2,305단어로 이미 충실**(서류·사전승인 vs 사전자격·딜러 활용법 전부 커버). 두 번째 사전승인 글은 카니발라이제이션 위험 → **각도를 딜러 금리 마크업으로 전환.**
- **중복 확인 결과**: 마크업/buy rate는 4개 글에 한두 문장씩 언급만 있고 **전용 페이지·계산기 0개**. 기존 툴과도 미중복(`zero-percent-apr-vs-rebate`는 0%vs리베이트, `car-refinance-calculator`는 기존 대출 리파이). 규칙 4 준수.

### 경쟁 회피 장치 (웹서치 1라운드)
- 경쟁: NerdWallet·Consumer Reports + carslens·vindxit·outsidefinancial·realcartips 등 다수. **전부 "마크업이란 무엇인가"를 설명하고 예시 숫자 하나($840/$1,600/$1,900)를 제시하는 데서 끝남.**
- **전용 계산기가 아무 데도 없음**(Redline에 money factor 계산기가 있으나 리스 전용). → 계산기로 진입.
- **추가 차별화 — 정직한 반대편을 넣음**: 경쟁 콘텐츠는 "딜러 금융 = 함정"으로 몰지만, **제조사 프로모(0%/1.9%)는 보조금이라 마크업 대상이 아니고 어떤 크레딧유니온도 0%를 못 이긴다.** 이 케이스를 계산기와 글 양쪽에 명시적으로 넣음. "사전승인의 역할은 쓰는 게 아니라 딜러가 이겨야 할 기준선을 만드는 것"이라는 프레임이 우리만의 각도.

### 검증된 사실·수치
- **MIT 2023 분석(NerdWallet 인용): 딜러 주선 대출의 약 78%가 마크업, 평균 1.13%p.**
- **Center for Responsible Lending: 평균 2.47%p($714 추가), 신차 1.01%p vs 중고차 2.91%p** — 중고차가 3배 가까이 심함.
- 렌더 정책상 마크업 상한 통상 2~2.5%p.
- 대출 $32,000 기준 추가이자: 1.13%p/60개월 $1,025, 2%p/60개월 **$1,825**(월 $30.41), 2%p/84개월 **$2,653**, 2.91%p/84개월 $3,894.
- **기간이 길수록 같은 마크업의 피해가 커짐**(2%p 기준 36개월 $1,058 → 84개월 $2,653).
- 0% vs 리베이트: $32,000/60개월에서 리베이트 $2,500이면 0%가 $2,965 유리, **리베이트 $5,000이면 두 안이 $2 차이로 거의 동일** — 리베이트 크기가 결론을 가른다.

### 신규 툴 — `tools/dealer-rate-markup-calculator.html` (1,008단어)
- 사전승인 금리 vs 딜러 제시 금리 + 대출액·기간·프로모 여부·포기 리베이트 입력.
- 3분기 출력: 양쪽 총비용 비교 / **마크업 카드**(스프레드 포인트, 월 추가액, 총 추가액, MIT·CRL 평균 대비 위치) / 해설.
- **분기 처리**: 프로모 선택 시 마크업 카드를 숨기고 리베이트 트레이드오프로 전환, 딜러가 더 쌀 때는 "사전승인이 제 역할을 했다"로 결론, 기간 72개월 이상이면 경고 추가.
- 검증: 파이썬 독립 구현 + node 목업 DOM 6시나리오 일치(기본/84개월/딜러우세/프로모+리베이트2500/리베이트5000/마크업3p).

### 신규 블로그 — `blog/dealer-financing-vs-pre-approval.html` (Financing, 1,191단어)
- 딜러 리저브 구조, 마크업×기간 비용표, **"딜러가 진짜로 이기는 경우"** 섹션(제조사 프로모·딜러가 더 많은 렌더를 조회), 사전승인의 진짜 역할, 딜러에서 대화 운영 순서 6단계, 이미 서명했다면 리파이낸스로 회수.
- 의사결정형 모양 유지.

### 사이트 반영
- blog/index.html Latest + Financing, tools/index.html Buying & Financing, index.html 미리보기 3건 교체 + 툴카드, stat `36+`→`37+`.
- sitemap 131→133 URL(lastmod 11건), llms.txt Tools 1 + Financing 1.
- 내부링크 12개 페이지(사전승인·금리·리파이낸스·협상·다운페이먼트 클러스터). 인바운드 8/11.

### 검증
- CSS 정규식 경계 추출 + 오염검사 통과. 17개 파일 태그 밸런스·JSON-LD 통과, sitemap XML 유효(133 URL), 모바일 `.row` 통과, 내부링크 1,280개 전수 스캔 깨짐 0건.

### ⚠️ 다음 세션 필독
1. **fill-up 페이지가 3번째 전환 엔진으로 급부상**(189→421노출, 5→9클릭). **차종 특정 주유비 쿼리가 계속 유입 중** — "2026 buick envision", "2026 xc90", "midsize suv", "gmc suburban". **차종/탱크용량을 입력받는 도구 형태로 접근할 것**(차종별 페이지 양산은 콘텐츠 팜 리스크). 다음 세션 1순위 후보.
2. **딜러 마크업 신규 2건의 효과를 다음 자료에서 확인** — 사전승인 페이지(3.59위)에서 내부링크를 걸었으므로 Bing 쪽 반응이 먼저 나올 가능성.
3. **연봉·다운페이먼트 클러스터 계속 확장.** 남은 각도: 가구 합산 소득, 첫 차 예산, 마일당 비용 프레임.
4. 판매세 과세표준 쿼리 클릭 발생("when you buy a new car is sales tax based off of the sales price") — `car-sales-tax-calculator`(Bing 31노출/1클릭/6.29위) 보강 여지.
5. **MPG 메타 3번째 수정 금지.** 874노출/2클릭은 쿼리 구조 탓.
6. 이월: 차량 기부 세금공제, 겨울타이어, Trucks 확장(트레일러 구입 vs 렌트·토우 패키지), Repairs 확장(타이어 2 vs 4·정비 견적 읽는 법), Accidents 확장(렌터카 커버리지·ACV 이의제기).
7. blog/index.html Latest 트리밍 여부 16세션째 미결.
