...
- **⚠️ 다음 세션 필독**:
  ...
  2. 이번 조치는 "발견됨-크롤링대기" 55개 중 링크 0개였던 9개만 우선 처리한 것. 나머지(1개만 받던 페이지 다수, EV/연비/로드트립 클러스터 등)는 이미 2~3개씩 받고 있어 상대적으로 급하지 않지만 아직 미색인 상태 — 이번 조치의 효과가 1~2주 후 Coverage 리포트에 반영되는지 먼저 확인한 뒤, 효과가 있으면 동일한 방식(색인된 페이지發 링크 보강)으로 나머지도 순차 처리할 것. 효과가 없으면 "내부링크 결손" 가설도 기각하고 도메인 권위/백링크 문제로 원점 재검토 필요.
  3. Related Guides 리스트에 항목을 추가할 때는 사이드바 리스트 삽입 방식(이번 세션)보다 본문 내 contextual 링크가 이론상 더 강한 시그널이라는 점 참고 — 리스트 삽입으로 효과가 부족하면 다음 단계로 본문 내 링크 삽입 고려.
