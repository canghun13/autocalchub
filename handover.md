...
11. **레이아웃/CSS 수정은 "다 됐다"고 보고하기 전에 실제로 반응형 크기별로 확인부터 할 것** — 코드만 보고 고친 뒤 던지지 말고, 최소한 좁은 뷰포트/모바일 폭 기준으로 깨지는지 직접 확인 후 보고. (7/13 footer-links overflow 건: 사용자가 CSS 문제라고 알려줬는데도 "링크 다 확인해야 하냐"고 재차 물어야 했음 — 사용자는 영어를 잘 못 읽고 필요한 것만 짚어주길 원함. 텍스트/콘텐츠 문제와 레이아웃/코드 문제를 명확히 구분해서, 코드만 고친 경우 "내용 확인 안 해도 됨, 이 부분만 화면으로 보면 됨"까지 먼저 말해줄 것.)
