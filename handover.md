...
## 5. SEO / Search Console 현황 (2026-07-27 기준 최신 확인 — 오늘 세션에서 실제 GSC export(Coverage+Performance zip) + GA4 개요 CSV로 재확인)

- **🎉 첫 클릭 발생**: 5/10 데이터 시작 이후 3개월 누적으로 **클릭 1건 발생**(7/25, 홈페이지 `/`에서, 노출 39회 중 클릭 1건, CTR 2.56%, 게재순위 10.95). 어떤 검색어였는지는 저빈도 쿼리 프라이버시 필터링으로 검색어 리포트에 안 뜸 — 특정 불가. 3개월 누적 클릭 1건 / 노출 1,728회(파이썬으로 차트.csv 합산 재확인).
- **그 외 전부 정체**: 아래 페이지들은 지난 확인(7/19) 대비 게재순위가 소수점까지 동일하거나 오차범위 내(±1위) — car-loan-calculator 4.07위(동일), road-trip-cost-calculator 9.53위(동일), how-much-car-can-i-afford 19.39위/노출110(동일), what-is-good-interest-rate-car-loan 19.16위(동일), car-insurance-estimator 85.85위(거의 동일), how-to-get-pre-approved-car-loan 77.1위(동일). car-down-payment-calculator만 7.75→8.6위로 소폭(표본 5회, 무의미), how-to-negotiate-car-price-at-dealership도 47.17→48.15위(표본 13회, 무의미). **3개월 롤링 평균 특성상 최근 몇 주 작업 효과가 반영되려면 아직 시간 필요.**
- **색인 현황**: 색인 생성됨 16개, 6/6부터 계속 정체(7주 이상). 미색인 55개(발견됨-크롤링대기 53 + 크롤링됨-미생성 2) — 7/15 확인 이후 그대로. Coverage 차트가 이번엔 **7/10 데이터까지만 반영**(지연 17일 — 지금까지 중 가장 길게 벌어짐). 즉 7/15 내부링크 보강부터 오늘(7/27, EV등록세+리스테이크오버) 발행분까지 전부 이 리포트에 아직 미반영 — 다음 확인은 최소 8/3 이후에나 일부 반영 기대.
- **심각한 문제 2건**: 리디렉션 페이지 1건, 대체 표준태그 페이지 1건 — 동일, 호스팅 레벨 이슈로 계속 무시.
- **의도적으로 방치 중인 영역**: `car-insurance-estimator.html`(노출 1,075회, 사이트 전체 1위 노출량, 85.85위) 포함 보험 계열 키워드 전체 — 상위 쿼리 전부 "car insurance calculator/estimate/estimator" 계열이며 80~94위대 정체, 여전히 무의미. 정책 계속 유지.
- **국가/기기**: 미국이 노출의 93%(1,615/1,728) 차지, 정상. 데스크톱 1,205 vs 모바일 522 — 데스크톱이 첫 클릭도 만듦(기기별 리포트에서 데스크톱만 클릭 1건 표시).

### GA4 애널리틱스 신규 확인 (6/29~7/26, 4주 — 이번이 처음으로 실제 GA 데이터 확인)
- 활성 사용자 51명, 신규 51명(재방문 사실상 0) — 저권위 신생 사이트 baseline으로 정상.
- **트래픽 소스 분해가 중요**: (direct) 39~46세션으로 압도적, 구글 오가닉은 3세션뿐(+bing 2, duckduckgo/yahoo 각 1 — 검색엔진 전체 합쳐도 7세션). **claude.ai / ai-assistant 경유가 6세션**으로 구글 오가닉보다 많음 — 출처 불명확(대화 내 링크 공유 등으로 추정), 특별 대응 불필요하지만 인지해둘 것.
- **⚠️ 향후 GA 데이터 해석 시 반드시 감안할 점**: 지역 데이터 1위가 Busan(5명)이고, 소스에 tryrating.com/foundrlist/Findly.tools/newtool.site 등 (7/15 세션에서 백링크용으로 등록한) 디렉토리 리퍼럴이 섞여 있음 — 이는 실제 검색 유입 사용자가 아니라 **본인 테스트 방문 + 디렉토리 크롤러/리뷰어 트래픽일 가능성이 높음**. 따라서 GA 페이지별 조회수 랭킹(홈페이지>Tools인덱스>Car Loan Calculator...)을 "인기 콘텐츠" 신호로 곧바로 해석하면 안 됨 — 검색 유입 트래픽이 아닌 잡음이 섞인 순서일 수 있음. 홈페이지 이탈률 57%, 일부 글 100%(단일 페이지뷰 후 이탈)도 이 해석과 일치. **다음에 GA 데이터로 "어떤 콘텐츠가 반응이 좋다"는 판단을 내릴 땐 이 트래픽 구성 문제를 먼저 감안할 것.**

### 이번 세션(7/27) 콘텐츠 작업과의 관계
- 오늘 GSC/GA 데이터는 첫 클릭 1건 외엔 전부 정체 상태라, 이미 진행한 EV 등록세/리스 테이크오버 콘텐츠 방향을 바꿀 새로운 신호는 없었음. 해당 콘텐츠는 키워드 리서치/경쟁강도 확인 근거로 이미 결정·발행 완료(아래 6-11 참고), 오늘 데이터는 이를 재확인/기각할 근거가 없어 추가 조정 없이 그대로 유지.
